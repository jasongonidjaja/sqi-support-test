...
const DeploymentBoardPage = () => {
  ...
  const handleDownload = async () => {
    try {
      ...
      const response = await api.get(
        `/deployment-requests/download/${filename}`,
        {
          responseType: 'blob',
        }
      );
  ...
  const dayCellContent = (arg) => {
    ...
    const deployment = dayEvents.filter(
      (e) => e.extendedProps.type === 'deployment'
    );
    const supports = dayEvents.filter(
      (e) => e.extendedProps.type === 'support'
    );
                  ...
                  <>
                    ...
                    {role === 'developer' && (
                      <Typography sx={{ mt: 1 }}>
                        ...
                        {sqiPics.find((p) => p.id === selectedEvent.sqiPicId)
                          ?.name || '-'}
                      </Typography>
                    )}
